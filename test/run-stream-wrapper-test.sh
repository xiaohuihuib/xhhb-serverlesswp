...
# Integration test for the serverlesswp-stream-wrapper wiring in api/index.js:
...
#   SERVERLESSWP_LOCAL=/path/to/serverlesswp-node ./run-stream-wrapper-test.sh
...
docker stop serverlesswp-stream-wrapper minio-stream-wrapper 2>/dev/null || true
docker rm serverlesswp-stream-wrapper minio-stream-wrapper 2>/dev/null || true
docker network rm serverlesswp-stream-wrapper-network 2>/dev/null || true
...
cleanup() {
    ...
    docker logs serverlesswp-stream-wrapper > /tmp/stream-wrapper-container.log 2>&1 || true
    echo "Full container log: /tmp/stream-wrapper-container.log"
    docker logs serverlesswp-stream-wrapper 2>&1 | tail -40 || true
    docker stop serverlesswp-stream-wrapper minio-stream-wrapper 2>/dev/null || true
    docker rm serverlesswp-stream-wrapper minio-stream-wrapper 2>/dev/null || true
    docker network rm serverlesswp-stream-wrapper-network 2>/dev/null || true
}
