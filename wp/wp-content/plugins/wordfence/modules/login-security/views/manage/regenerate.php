	...
	<div class="wfls-block-header wfls-block-header-border-bottom">
		<div class="wfls-block-header-content">
			<div class="wfls-block-title wfls-two-factor-card-title">
				<span class="wfls-two-factor-card-title-icon" aria-hidden="true"><i class="<?php echo esc_attr(\WordfenceLS\Utility_Style::font_awesome_classes('key')); ?>"></i></span>
				<strong><?php esc_html_e('Recovery Codes', 'wordfence'); ?></strong>
			</div>
		</div>
	</div>
	<div class="wfls-block-content wfls-padding-add-bottom">
		<p id="wfls-recovery-code-count"><?php echo esc_html(sprintf(/* translators: count */ _n('%d unused recovery code remains. You may generate a new set by clicking below.', '%d unused recovery codes remain. You may generate a new set by clicking below.', $remaining, 'wordfence'), $remaining)); ?></p>
...
<div style="display: none;">
	...
	echo \WordfenceLS\Model_View::create('common/modal-prompt', array(
		...
		'primaryButton' => array('class' => 'wfls-recovery-prompt-cancel', 'label' => __('Cancel', 'wordfence'), 'link' => '#'),
		'secondaryButtons' => array(array('class' => 'wfls-recovery-prompt-confirm', 'label' => __('Generate', 'wordfence'), 'link' => '#')),
	))->render();
	?>
</div>
	...
	(function($) {
		$(function() {
			$('#wfls-recovery').on('click', function(e) {
				...
				var content = $("#wfls-template-recovery-prompt").clone().attr('id', null);

				WFLS.standaloneModalHTML(content, {
					onOpen: function(modal) {
						$(modal).find('.wfls-recovery-prompt-cancel').on('click', WFLS.closeStandaloneModal);
						$(modal).find('.wfls-recovery-prompt-confirm').on('click', function(e) {
							e.preventDefault();
							e.stopPropagation();
							...
							WFLS.ajax(
								...
								function(response) {
									if (response.error) {
										WFLS.standaloneModal(<?php \WordfenceLS\Text\Model_JavaScript::echo_string_literal(__('Error Generating New Codes', 'wordfence')); ?>, response.error);
									}
										...
										var recoveryCodeFileContents = '<?php echo \WordfenceLS\Text\Model_JavaScript::esc_js(sprintf(/* translators: 1. Site address; 2. Username */ __('Two-Factor Authentication Recovery Codes - %1$s (%2$s)', 'wordfence'), preg_replace('~^https?://~i', '', home_url()), $user->user_login)); ?>' + "\r\n";
										...
										message = message + "<p class=\"wfls-center\"><a href=\"#\" class=\"wfls-btn wfls-btn-default wfls-recovery-new-download\" target=\"_blank\" rel=\"noopener noreferrer\"><i class=\"dashicons dashicons-download\"></i> <?php echo \WordfenceLS\Text\Model_JavaScript::esc_js(__('Download', 'wordfence')); ?></a></p>";

										WFLS.standaloneModal(<?php \WordfenceLS\Text\Model_JavaScript::echo_string_literal(__('New Recovery Codes', 'wordfence')); ?>, { html: message }, {
											onOpen: function(recoveryCodeModal) {
												$(recoveryCodeModal).find('.wfls-recovery-new-download').on('click', function(e) {
													e.preventDefault();
													e.stopPropagation();
								...
								function(error) {
									WFLS.standaloneModal('<?php echo \WordfenceLS\Text\Model_JavaScript::esc_js(__('Error Generating New Codes', 'wordfence')); ?>', '<?php echo \WordfenceLS\Text\Model_JavaScript::esc_js(__('An error was encountered while trying to generate new recovery codes. Please try again.', 'wordfence')); ?>');
								}
							);
						});
					}
